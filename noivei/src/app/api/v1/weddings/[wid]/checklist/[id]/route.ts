...
import { UpdateChecklistItemSchema } from '@/lib/api/validation/checklist.schema'
...
export async function PATCH(req: Request, { params }: RouteContext) {
  try {
    ...
    if (!UuidSchema.safeParse(id).success) {
      return err(404, 'ITEM_NOT_FOUND', 'Item do checklist não encontrado.')
    }
    ...
    if (error) return err(500, 'DB_ERROR', 'Erro ao atualizar item do checklist.')
    if (!data) return err(404, 'ITEM_NOT_FOUND', 'Item do checklist não encontrado.')
...
export async function DELETE(_req: Request, { params }: RouteContext) {
  try {
    ...
    if (error) return err(500, 'DB_ERROR', 'Erro ao remover item do checklist.')
    if (!data) return err(404, 'ITEM_NOT_FOUND', 'Item do checklist não encontrado.')
